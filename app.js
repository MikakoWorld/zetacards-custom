...
function drawContainedText(ctx,text,box,opt={}){const o={maxFontSize:38,minFontSize:18,fontWeight:700,color:'#23233a',lineHeight:1.35,align:'left',valign:'top',paddingX:0,paddingY:0,...opt},content=String(text||'').trim();if(!content)return;let fs=o.maxFontSize,lines=[];while(fs>=o.minFontSize){ctx.font=`${o.fontWeight} ${fs}px system-ui,-apple-system,sans-serif`;lines=wrapText(ctx,content,Math.max(1,box.w-o.paddingX*2));const h=lines.length*fs*o.lineHeight;if(h<=Math.max(1,box.h-o.paddingY*2))break;fs--}ctx.font=`${o.fontWeight} ${fs}px system-ui,-apple-system,sans-serif`;ctx.fillStyle=o.color;ctx.textAlign=o.align;ctx.textBaseline='top';const total=lines.length*fs*o.lineHeight;let startY=box.y+o.paddingY;if(o.valign==='middle')startY=box.y+(box.h-total)/2;if(o.valign==='bottom')startY=box.y+box.h-total-o.paddingY;let x=box.x+o.paddingX;if(o.align==='center')x=box.x+box.w/2;if(o.align==='right')x=box.x+box.w-o.paddingX;lines.forEach((line,i)=>ctx.fillText(line,x,startY+i*fs*o.lineHeight))}
...
drawContainedText(ctx,state.profile.name,boxByPercent(w,h,50.5,11,41.2,4),{maxFontSize:44,minFontSize:22,valign:'middle'});
drawContainedText(ctx,state.profile.xId,boxByPercent(w,h,50.5,15.3,41,4),{maxFontSize:44,minFontSize:22,valign:'middle'});
drawContainedText(ctx,state.profile.zetaHistory,boxByPercent(w,h,50.5,19.5,41,3.6),{maxFontSize:44,minFontSize:22,valign:'middle'});
drawContainedText(ctx,state.profile.creatorId,boxByPercent(w,h,62,23.8,29.5,4.2),{maxFontSize:40,minFontSize:20,valign:'middle'});
drawContainedText(ctx,state.profile.nickname,boxByPercent(w,h,13,30.6,32,3),{maxFontSize:40,minFontSize:20,align:'center',valign:'middle'});
drawContainedText(ctx,state.profile.favoriteThing,boxByPercent(w,h,37,34.5,32,3),{maxFontSize:40,minFontSize:20,align:'center',valign:'middle'});
...
drawContainedText(ctx,state.profile[`oshi${slot.n}Name`],boxByPercent(w,h,...slot.name),{maxFontSize:34,minFontSize:18,align:'center',valign:'middle'});
drawContainedText(ctx,state.profile[`oshi${slot.n}Desc`],boxByPercent(w,h,...slot.desc),{maxFontSize:22,minFontSize:12,lineHeight:1.45});
}
}
